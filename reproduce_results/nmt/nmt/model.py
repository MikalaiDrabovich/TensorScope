...
opts = (option_builder.ProfileOptionBuilder(tf.profiler.ProfileOptionBuilder.time_and_memory())
        .with_max_depth(10000)
        .account_displayed_op_only(False)
        .select(['op_types', 'params','occurrence', 'micros', 'accelerator_micros','cpu_micros', 'device', 'input_shapes', 'params', 'float_ops'])
        .order_by('micros')# order_by micros|accelerator_micros|cpu_micros float_ops occurren
        .with_step(-1)
        .with_file_output('/tmp/tensorscope/ptb_tfprof.txt')
        #.with_min_micros(0)
        #.with_timeline_output('/tmp/tensorscope/ptb_tfprof_timeline.json')
        .build())
...
class BaseModel(object):
  ...
  def train(self, sess):
    ...
    if astep >=2 : # skip first 2 steps for 'warm up'
      profiler.add_step(astep-2, run_meta)
      print('added step for profiling: ', astep)
      print(astep)
